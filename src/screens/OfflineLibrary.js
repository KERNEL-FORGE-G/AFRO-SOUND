...
export default function OfflineLibrary({navigation}) {
  ...
  const renderTrack = ({item}) => (
    <TouchableOpacity
      ...
      onPress={() => playTrack({id: item.id, url: `file://${item.path}`, title: item.title})}>
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>Aucun son téléchargé.</Text>}
        />
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  container: {flex: 1, backgroundColor: Colors.background, padding: Spacing.md, paddingTop: 50},
  title: {color: Colors.text, fontSize: Typography.hero, fontWeight: '800', marginBottom: 20},

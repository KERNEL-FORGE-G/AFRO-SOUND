...
function build() {
    ...
    execSync('npx tailwindcss -i ./dashboard/input.css -o ./dashboard/output.css', { cwd: rootDir });

    // 2. Run Babel
    console.log('Running Babel...');
    execSync('npx babel ./dashboard/App.jsx --out-file ./dashboard/App.js', { cwd: rootDir });
    ...
    let customCss = fs.readFileSync(path.join(dashboardDir, 'styles.css'), 'utf8');
    ...
    html = html.replace('<!-- DASHBOARD_JS -->', '<script>\n' + js + '\n</script>');
